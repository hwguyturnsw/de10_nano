#!/bin/bash
#
#Write SD card for Cyclone V SoC FPGA buildroot image and auto resize the rootfs at image write time.
#ejc-01.06.2024
...
select opt in Main-Image Release-Image Custom-Image-Dir Clean-Disk Exit; do
	case $opt in
		"Main-Image")
			...
			dd if=../buildroot-2023-02.02/output/images/sdcard.img of=/dev/$sd_mount_point status=progress
			break
			;;
		"Release-Image")
			...
			dd if=../release/cyclonev_$release_ver/cyclonev_$release_ver\_$release_date\_signed/sdcard.img of=/dev/$sd_mount_point status=progress
			break
			;;
		"Custom-Image-Dir")
